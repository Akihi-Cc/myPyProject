...
# 2、readlines() - 读取文件的全部内容，它会把读取的内容赋值给一个列表变量。
from urllib.request import urlopen
...
f.write(content)
f.close()
# 行以上代码，在本地就会生成一个 runoob_urllib_test.html 文件，里面包含了 https://www.runoob.com/ 网页的内容。
...
# 单 POST 传递数据，我们先创建一个表单，代码如下，我这里使用了 PHP 代码来获取表单的数据：
...
fh.write(reponse)
fh.close()
# 行以上代码，会提交表单数据到 py3_urllib_test.php 文件，输出结果写入到 urllib_test_post_runoob.html 文件。
# 打开 urllib_test_post_runoob.html 文件（可以使用浏览器打开）

print("【3】 urllib.error *******************************************")
