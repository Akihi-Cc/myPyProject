...
#             BufferError：缓冲区相关操作错误。
#             EOFError：输入结束时没有读取到数据。
...
#                 KeyError：映射中没有此键。
#             MemoryError：内存不足。
#             NameError：未找到局部或全局名称。
#                 UnboundLocalError：尝试访问未初始化的局部变量。
#             OSError：操作系统错误。
#                 BlockingIOError：阻塞操作被中断。
#                 ChildProcessError：子进程上的操作失败。
#                 ConnectionError：连接相关错误。
#                     BrokenPipeError：管道损坏。
#                     ConnectionAbortedError：连接建立后被强制关闭。
#                     ConnectionRefusedError：连接请求被拒绝。
#                     ConnectionResetError：连接已重置。
#                 FileExistsError：尝试创建已存在的文件或目录。
#                 FileNotFoundError：找不到文件或目录。
#                 InterruptedError：系统调用被信号中断。
#                 IsADirectoryError：当预期文件时遇到目录。
#                 NotADirectoryError：当预期目录时遇到文件。
#                 PermissionError：权限不足。
#                 ProcessLookupError：指定进程不存在。
#                 TimeoutError：超出时间限制。
#             ReferenceError：弱引用试图访问已经垃圾回收的对象。
#             RuntimeError：运行时错误，通常用于其他类别不适用的情况。
#                 NotImplementedError：尚未实现的方法或操作。
#             SyntaxError：语法错误。
#                 IndentationError：缩进错误。
#                     TabError：制表符和空格混合使用导致的缩进错误。
#             SystemError：解释器系统错误。
...
#                 FutureWarning：未来版本中将更改的行为警告。
#                 ImportWarning：导入过程中可能的问题警告。
#                 UnicodeWarning：Unicode 相关的警告。
#                 BytesWarning：字节相关的警告。
#                 ResourceWarning：资源使用相关的警告。
#         SystemExit：解释器请求退出。
#         KeyboardInterrupt：用户中断执行（通常是按下 Ctrl+C）。
#         GeneratorExit：生成器的关闭请求。
